...
pub struct ModelTableMetadata {
    ...
    pub generation_exprs: Vec<Option<Expr>>,
}
...
impl ModelTableMetadata {
    ...
    pub fn try_new(
        ...
        generation_exprs: Vec<Option<Expr>>,
        ...
        // If an generation expression is not defined for each column, return an error.
        if schema.fields().len() != generation_exprs.len() {
            return Err(ModelarDbError::ConfigurationError(
                "An genration expression must be defined for each column.".to_owned(),
            ));
        }
...
mod test {
    ...
    fn test_can_create_model_table_metadata() {
        let (schema, error_bounds, generation_exprs) = model_table_schema_and_error_bounds();
    ...
    fn test_cannot_create_model_table_metadata_with_missing_or_too_many_error_bounds() {
        let (schema, _error_bounds, generation_exprs) = model_table_schema_and_error_bounds();
        let result = ModelTableMetadata::try_new(
            "table_name".to_owned(),
            schema,
            3,
    ...
    #[test]
    fn test_cannot_create_model_table_metadata_with_missing_or_too_many_generation_expr() {
        let (schema, error_bounds, _generation_exprs) = model_table_schema_and_error_bounds();
        let result = ModelTableMetadata::try_new(
            "table_name".to_owned(),
            schema,
            3,
    ...
    fn model_table_schema_and_error_bounds() -> (Schema, Vec<ErrorBound>, Vec<Option<Expr>>) {
        (
            Schema::new(vec![
