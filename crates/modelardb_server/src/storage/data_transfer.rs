...
impl DataTransfer {
    ...
    /// `local_data_folder_path` could not be read, return [`ModelarDbServerError`].
    ...
    /// successfully, otherwise [`ModelarDbServerError`]. Note that if the function fails, some of
    /// the compressed files may still have been transferred. Since the data is transferred
    /// separately for each table, the function can be called again if it failed.
    ...
    /// files were transferred successfully, otherwise [`ModelarDbServerError`].
