...
impl UncompressedDataManager {
    ...
    async fn insert_data_points(&self, ingested_data_buffer: IngestedDataBuffer) -> Result<()> {
        ...
        let time_series_table_metadata = ingested_data_buffer.time_series_table_metadata;
        ...
        let (timestamp_column_array, field_column_arrays, tag_column_arrays) =
            time_series_table_metadata.column_arrays(&data_points)?;

        // For each data point, insert the timestamp and values into the corresponding UncompressedDataBuffer.
        for (index, maybe_timestamp) in timestamp_column_array.iter().enumerate() {
            ...
            let timestamp =
                maybe_timestamp.expect("timestamp_column_array should not contain null values.");
    ...
    async fn insert_data_point(
        ...
        if !buffer_is_in_memory {
            ...
            buffers_are_spilled |= self
                .reserve_uncompressed_memory_for_in_memory_data_buffer(
                    time_series_table_metadata.field_column_indices.len(),
                )
                .await?;
        ...
        if buffer_is_full {
            ...
            let (_tag_hash, full_uncompressed_in_memory_data_buffer) = self
                ...
                .remove(&tag_hash)
                .expect("If the buffer is full it should exist in the DashMap.");
    ...
    async fn spill_in_memory_data_buffer(&self) -> Result<()> {
        ...
        let mut uncompressed_in_memory_data_buffer = self
            .uncompressed_in_memory_data_buffers
            .remove(&tag_hash)
            .expect("tag_hash should exist since it was just extracted from the DashMap.")
            .1;
    ...
    async fn compress_finished_buffer(
        ...
        let compressed_segments = field_column_arrays
            .iter()
            .zip(time_series_table_metadata.field_column_indices.iter())
            .map(|(uncompressed_values, field_column_index)| {
                ...
                .expect(
                    "uncompressed_timestamps and uncompressed_values should have the same length.",
                )
            })
            .collect::<Vec<_>>();
...
mod tests {
    ...
    async fn test_can_insert_data_point_into_new_uncompressed_data_buffer() {
        ...
        insert_data_points(1, &mut data_manager, &time_series_table_metadata, TAG_VALUE).await;
    ...
    fn test_remaining_memory_not_incremented_when_processing_on_disk_buffer() {
        ...
        let uncompressed_data = table::uncompressed_time_series_table_record_batch(3);
    ...
    /// Create a [`DataFolder`] with a time series table saved to it and an [`UncompressedDataManager`]
