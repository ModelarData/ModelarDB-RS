...
impl MetadataDeltaLake {
    ...
    /// remote object store given by `connection_info`. Returns [`ModelarDbStorageError`] if
    ...
    pub fn try_from_s3_configuration(
        ...
        let url = Url::parse(&location).map_err(|error| {
            ModelarDbStorageError::InvalidArgument(format!("Unable to parse S3 location: {error}"))
        })?;
    ...
    /// registered, return [`ModelarDbStorageError`].
    ...
    /// return the updated [`DeltaTable`], otherwise return [`ModelarDbStorageError`].
    ...
    /// [`DeltaOps`] cannot be retrieved, return [`ModelarDbStorageError`].
    ...
    /// return a [`RecordBatch`] with the query result, otherwise return [`ModelarDbStorageError`].
    ...
    /// metadata Delta Lake, or a [`ModelarDbStorageError`] if a connection cannot be established or
