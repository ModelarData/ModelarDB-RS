...
| `--vacuum-retention-period-in-seconds`    | `MODELARDBD_VACUUM_RETENTION_PERIOD_IN_SECONDS`    | 7 days      | ✓             | The retention period used when automatically vacuuming a table after optimization. Also used as the default when `VACUUM` is run without an explicit `RETAIN`. Note that a very low value can delete files an in-flight query is still scanning. |
