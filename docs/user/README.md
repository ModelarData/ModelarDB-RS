...
| `--ingested-reserved-memory`              | `MODELARDBD_INGESTED_RESERVED_MEMORY`              | 10 %        | ✓             | The amount of memory to reserve for storing ingested time series.                                                                                                                                                                                 |
| `--uncompressed-reserved-memory`          | `MODELARDBD_UNCOMPRESSED_RESERVED_MEMORY`          | 10 %        | ✓             | The amount of memory to reserve for storing uncompressed data buffers.                                                                                                                                                                            |
| `--compressed-reserved-memory`            | `MODELARDBD_COMPRESSED_RESERVED_MEMORY`            | 30 %        | ✓             | The amount of memory to reserve for storing compressed data buffers.                                                                                                                                                                              |
...
| `--optimize-target-file-size-in-bytes`    | `MODELARDBD_OPTIMIZE_TARGET_FILE_SIZE_IN_BYTES`    | 64 MB       | ✓             | The target size of the files produced when automatically compacting a table's storage. Also used as the default when `OPTIMIZE` is run without an explicit `TARGET`.                                                                              |
